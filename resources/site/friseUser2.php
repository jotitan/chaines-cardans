...
$iduser = $_GET["idUser"];
$dao = new UserDao();
$tab = $dao->getMotosOfUser($iduser,false);
...
if(sizeof($tab)>0){
	...
	for($i=0;$i<sizeof($tableaux);$i++){
		...
		if($tableaux[$i]->getInfo("debut")!=null){
			...
			// On dessine deux demi arc de cercle si la largeur du rectangle est superieur a la hauteur + 10)
			if($tableaux[$i]->getInfo("longueur") > $epaisseur +5){
				ImageFilledRectangle($image,15 + $pos + $epaisseur/2,$posVertical,15 + $pos + $tableaux[$i]->getInfo("longueur") - $epaisseur/2,$posVertical + $epaisseur-1,$col);				
				imagefilledarc ( $image , 15 + $pos + $epaisseur/2 , $posVertical + $epaisseur/2, $epaisseur , $epaisseur , 90 , 270 , $col , IMG_ARC_PIE);
				imagefilledarc ( $image , 15 + $pos + $tableaux[$i]->getInfo("longueur") - $epaisseur/2 , $posVertical + $epaisseur/2, $epaisseur , $epaisseur , 270 , 90 , $col , IMG_ARC_PIE);
			}
			else{
				ImageFilledRectangle($image,15 + $pos,$posVertical,15 + $pos + $tableaux[$i]->getInfo("longueur"),$posVertical + $epaisseur-1,$col);
			}
			
			/* On enregistre ces coordonnes dans la base */
			if($tab[$i]->getInfo("id")!=""){
			$moto = $dao->getMoto($tab[$i]->getInfo("id"));
				if($moto!=null){
					...
					$areaResponse.= ((!$first)?",":"") . sprintf("{\"name\":\"%s\",\"commentaire\":\"%s\",\"color\":\"%s\",\"coords\":\"%s\",\"path\":\"%s\"}",
						$name,$tab[$i]->getInfo("commentaire"),$color,$coords,$path);
					$first = false;
				}			
			}
		}
